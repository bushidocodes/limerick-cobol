...
| ----------------------------------------------------------- | ------------------------------------------------------------------------ |
| `examples/**/*.html`                                        | `scripts/build-examples.js` (template + manifest) + sibling `*.cbl` file |
| `sitemap.xml`                                               | `scripts/build-sitemap.js` (driven by `git log`)                         |
| `search-index.json`                                         | `scripts/build-search-index.js` (driven by page `<title>` + meta)        |
| `pics/og/*.png`                                             | `scripts/generate-og-images.js`                                          |
| `favicon.*`, `apple-touch-icon.png`, `android-chrome-*.png` | `scripts/generate-icons.js` (driven by the logo SVG)                     |
| `course/lesson-meta.json`                                   | `scripts/generate-lesson-jsonld.js`                                      |
...
Several other scripts modify HTML files **in place** — `add-img-dimensions`, `add-prev-next-links`, `add-reading-time`, `inject-cross-links`, `inject-site-header`, `inject-theme-bootstrap`, `fix-related-content-placement`. These are idempotent (re-running adds nothing if the target attribute / element already exists), so manual edits to existing markup outside the script's scope are preserved.
