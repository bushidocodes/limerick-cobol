...
These paths are **build output** — do not edit them directly. CI regenerates them on every PR, so manual edits get clobbered. Edit the source listed in the right column, then re-run the build script.
...
Each generated HTML file carries a leading `<!-- Generated by … -->` comment as a foot-gun guard. If you find yourself editing one of these files to change a page-shell concern (favicon, meta tags, script imports, layout), edit the corresponding template in `scripts/` instead and re-run the script.

Several other scripts modify HTML files **in place** — `add-img-dimensions`, `add-prev-next-links`, `add-exercise-prev-next-links`, `add-reading-time`, `generate-exercise-jsonld`, `inject-cross-links`, `inject-site-header`, `inject-theme-bootstrap`. These are idempotent (re-running adds nothing if the target attribute / element already exists), so manual edits to existing markup outside the script's scope are preserved.
...
`npm run check` runs the full suite (typecheck, examples, search-index, validate, assets, img-dims, links, a11y) — use it before opening a PR when multiple generators may be affected.
