 ...
 * every PR). Page-shell changes — meta tags, script imports, layout — must
 * be made in the `buildPage()` template below. Per-example content (title,
 * crumb, description, related links) lives in the manifest in this file.
 *
...
const MANIFEST = [
	...
	// ── SubProg/DayDiff (depth 3) ────────────────────────────────────────────
	{
		...
		runInCe: "../DateValid/Validate.cbl",
	},
	// ── SubProg/Multiply (depth 3) ───────────────────────────────────────────
	{
		...
		runInCe: "MultiplyNums.cbl Fickle.cbl Steadfast.cbl",
	},
	{
...
function buildPage(entry) {
	...
	const runInCeAttr = typeof entry.runInCe === "string" ? ` extra="${escapeAttr(entry.runInCe)}"` : "";
	const runInCeScript = entry.runInCe ? `\t\t<script src="${pfx}components/run-in-ce.js" defer></script>\n` : "";
	const runInCeToolbarEl = entry.runInCe ? `\t\t\t\t\t\t\t<run-in-ce${runInCeAttr}></run-in-ce>\n` : "";

	// The leading HTML comment is intentional: every example page is
	// regenerated by this script, so manual edits here get clobbered the next
